...
router.put('/:id/status', async (req, res) => {
  try {
    ...
    if (status === 'accepted') {
    ...
    else if (status === 'late-returned') {
      await Notification.create({ 
        user: booking.user, 
        message: `🚨 ALERT: Your booking for ${booking.gearName} is OVERDUE (Late)! Please return it immediately.`, 
        type: 'alert' 
      });
     ...
     await Notification.create({ user: booking.user, message: `Thank you! Your booking for ${booking.gearName} has been safely returned.`, type: 'booking' });
    }
    res.json(booking);
  } catch (err) {
    res.status(500).json({ error: err.message });
